    ...
    for i in 0..canvas_h {
        ...
        for j in 0..canvas_w {
            ...
            if GRAY_MODE {
                ...
                // color mode
                let v = iter_index as f64 / max_iter as f64;
                data.push((9.0 * (1.0 - v) * v * v * v * 255.0) as u8); // R
                data.push((15.0 * (1.0 - v) * (1.0 - v) * v * v * 255.0) as u8); // G
                data.push((8.5 * (1.0 - v) * (1.0 - v) * (1.0 - v) * v * 255.0) as u8); // B
                data.push(255); // A
            }
        }
    }
